...
namespace DotNetty.Common.Utilities
{
    ...
    public static class CharUtil
    {
        ...
        const char MinHighSurrogate = '\uD800';
        const char MaxHighSurrogate = '\uDBFF';
        const char MinLowSurrogate = '\uDC00';
        const char MaxLowSurrogate = '\uDFFF';
        ...
        public static ICharSequence[] Split(ICharSequence sequence, int startIndex, params char[] separator)
        {
            ...
            var result = new List<ICharSequence>();
            ...
            while (i < length)
            {
                while (i < length && IndexOf(separator, sequence[i]) >= 0)
                {
                    i++;
                }
                ...
                if (i < length)
                {
                    ...
                    {
                        if (IndexOf(separator, sequence[position]) >= 0)
                        {
                            result.Add(sequence.SubSequence(i, position));
                            break;
                        }
                    }
        ...
        internal static bool SequenceEquals(ICharSequence left, ICharSequence right, bool ignoreCase)
        {
        ...
        public static bool RegionMatches(string value, bool ignoreCase, int thisStart, ICharSequence other, int start, int length)
        {
            Contract.Requires(value != null && other != null);

            if (!ignoreCase)
            {
                return RegionMatches(value, thisStart, other, start, length);
            }
        ...
        public static bool RegionMatches(IReadOnlyList<char> value, bool ignoreCase, int thisStart, ICharSequence other, int start, int length)
        {
            Contract.Requires(value != null && other != null);
        ...
        public static ICharSequence Trim(ICharSequence sequence)
        {
            ...
            int length = sequence.Count;
            int start = IndexOfFirstNonWhiteSpace(sequence, length);
            if (start == length)
            {
                return StringCharSequence.Empty;
            }

            int last = IndexOfLastNonWhiteSpaceChar(sequence, start, length);
        ...
        public static int IndexOfFirstNonWhiteSpace(IReadOnlyList<char> value)
        {
            Contract.Requires(value != null);

            return IndexOfFirstNonWhiteSpace(value, value.Count);
        }

        public static int IndexOfFirstNonWhiteSpace(IReadOnlyList<char> value, int length)
        {
            int i = 0;
            while (i < length && char.IsWhiteSpace(value[i]))
            {
                i++;
            }
        ...
        public static int IndexOfLastNonWhiteSpaceChar(IReadOnlyList<char> value, int start, int length)
        {
            int i = length - 1;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static bool IsISOControl(char c) => (c >= 0 && c <= 0x1f) || (c >= 0x7f && c <= 0x9f);
        ...
        public static int IndexOf(this ICharSequence cs, char searchChar, int start)
        {
            ...
            var sequence = cs as StringCharSequence;
            if (sequence != null)
            {
                return sequence.IndexOf(searchChar, start);
            }

            var s = cs as AsciiString;
            if (s != null)
            {
        ...
        public static int IndexOf(char[] tokens, char value)
        {
        ...
        public static int CodePointAt(IReadOnlyList<char> seq, int index)
        {
            ...
            return IsSurrogatePair(high, low) 
        ...
        static bool IsHighSurrogate(char ch) => 
            (MinHighSurrogate <= ch && MaxHighSurrogate >= ch);

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static bool IsLowSurrogate(char ch) => 
            (MinLowSurrogate <= ch && MaxLowSurrogate >= ch);
    }
}
