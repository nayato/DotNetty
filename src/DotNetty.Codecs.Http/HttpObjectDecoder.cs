...
namespace DotNetty.Codecs.Http {
    ...
    /**
 * Decodes {@link IByteBuffer}s into {@link HttpMessage}s and
 * {@link HttpContent}s.
 *
...
public abstract class HttpObjectDecoder : ByteToMessageDecoder {
    private static readonly string EMPTY_VALUE = "";

    private readonly int maxChunkSize;
    private readonly bool chunkedSupported;
    ...
    private long contentLength = long.MinValue;
    ...
    // @Override
    protected void decode(IChannelHandlerContext ctx, IByteBuffer buffer, List<object> output)  {
        if (resetRequested) {
            resetNow();
        }
        ...
        case READ_CHUNK_DELIMITER: {
            readonly int wIdx = buffer.writerIndex();
    ...
    // @Override
    protected void decodeLast(IChannelHandlerContext ctx, IByteBuffer input, List<object> output)  {
        base.decodeLast(ctx, input, output);
        ...
        if (message != null) {
            bool chunked = HttpUtil.isTransferEncodingChunked(message);
            if (currentState == State.READ_VARIABLE_LENGTH_CONTENT && !input.isReadable() && !chunked) {
                // End of connection.
                output.Add(LastHttpContent.EMPTY_LAST_CONTENT);
                reset();
                return;
    ...
    private HttpMessage invalidMessage(IByteBuffer input, Exception cause) {
    ...
    private HttpContent invalidChunk(IByteBuffer input, Exception cause) {
        currentState = State.BAD_MESSAGE;
    ...
    private static bool skipControlCharacters(IByteBuffer buffer) {
        bool skiped = false;
        readonly int wIdx = buffer.writerIndex();
        int rIdx = buffer.readerIndex();
        while (wIdx > rIdx) {
    ...
    private State readHeaders(IByteBuffer buffer) {
        readonly HttpMessage message = this.message;
        readonly HttpHeaders headers = message.headers();
    ...
    private LastHttpContent readTrailingHeaders(IByteBuffer buffer) {
        AppendableCharSequence line = headerParser.parse(buffer);
        if (line == null) {
            return null;
        }
    ...
    private class HeaderParser : ByteProcessor {
        private readonly AppendableCharSequence seq;
        ...
        public AppendableCharSequence parse(IByteBuffer buffer) {
            readonly int oldSize = size;
            seq.reset();
    ...
    private sealed class LineParser : HeaderParser {
