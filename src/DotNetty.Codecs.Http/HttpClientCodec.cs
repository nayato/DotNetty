...
namespace DotNetty.Codecs.Http
{
        ...
        HttpClientUpgradeHandler.ISourceCodec
    {
        ...
        public HttpClientCodec(int maxInitialLineLength = 4096, int maxHeaderSize = 8192, int maxChunkSize = 8192, bool failOnMissingResponse = false, bool validateHeaders = true)
        {
            this.Init(new Decoder(this, maxInitialLineLength, maxHeaderSize, maxChunkSize, validateHeaders), new Encoder(this));
            this.failOnMissingResponse = failOnMissingResponse;
        }

        public HttpClientCodec(int maxInitialLineLength, int maxHeaderSize, int maxChunkSize, bool failOnMissingResponse, bool validateHeaders, int initialBufferSize)
        {
        ...
        public void PrepareUpgradeFrom(IChannelHandlerContext ctx) => this.upgraded = true;
        ...
        sealed class Encoder : HttpRequestEncoder
        {
            ...
            protected override void Encode(IChannelHandlerContext context, object message, List<object> output)
            {
                ...
                if (request != null && !this.clientCodec.done)
                {
                ...
                if (this.clientCodec.failOnMissingResponse)
                {
                    ...
                    if (missingResponses > 0)
                    {
                        ctx.FireExceptionCaught(
                            new PrematureChannelClosureException($"channel gone inactive with {missingResponses} missing response(s)"));
                    }
                }
            }
        }
