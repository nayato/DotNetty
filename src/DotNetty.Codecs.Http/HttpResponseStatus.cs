...
namespace DotNetty.Codecs.Http
{
    ...
    public class HttpResponseStatus : IComparable<HttpResponseStatus>
    {
        ...
        static HttpResponseStatus NewStatus(int statusCode, string reasonPhrase) => 
            new HttpResponseStatus(statusCode, new AppendableCharSequence(reasonPhrase.ToCharArray()) , true);

        readonly byte[] bytes;

        HttpResponseStatus(int code) 
            : this(code, new AppendableCharSequence(
                $"{HttpStatusClass.ValueOf(code).DefaultReasonPhrase} ({code})".ToCharArray()))
        {
        }
        ...
        internal void Encode(IByteBuffer buf)
        {
            if (this.bytes == null)
            {
                ...
                buf.WriteCharSequence(new AsciiString(this.ReasonPhrase), Encoding.ASCII);
            }
            else
            {
