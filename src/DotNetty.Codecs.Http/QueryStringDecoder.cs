...
public class QueryStringDecoder {
    ...
    private static readonly int DEFAULT_MAX_PARAMS = 1024;

    private readonly Charset charset;
    private readonly string uri;
    private readonly bool hasPath;
     ...
     * specified charset.
     */
    public QueryStringDecoder(string uri, bool hasPath) {
     ...
     * specified charset.
     */
    public QueryStringDecoder(string uri, Charset charset, bool hasPath) {
     ...
     * specified charset.
     */
    public QueryStringDecoder(string uri, Charset charset, bool hasPath, int maxParams) {
        if (uri == null) {
    ...
    private bool addParam(Map<string, List<string>> params, string name, string value) {
     ...
     * escape sequence.
     */
    public static string decodeComponent(readonly string s) {
    ...
    public static string decodeComponent(string s, Charset charset) {
        ...
        readonly int size = s.length();
        bool modified = false;
        for (int i = 0; i < size; i++) {
            readonly char c = s.charAt(i);
            if (c == '%' || c == '+') {
                modified = true;
                break;
            }
        ...
        readonly byte[] buf = new byte[size];
        ...
        for (int i = 0; i < size; i++) {
            ...
            switch (c) {
                ...
                case '%':
                    if (i == size - 1) {
                        throw new ArgumentException("unterminated escape"
                    ...
                    if (i == size - 1) {
                        throw new ArgumentException("partial escape"
                                + " sequence at end of string: " + s);
                    }
                    c = decodeHexNibble(c);
                    readonly char c2 = decodeHexNibble(s.charAt(++i));
