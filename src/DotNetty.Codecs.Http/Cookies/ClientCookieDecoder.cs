...
namespace DotNetty.Codecs.Http.Cookies
{
    ...
    public sealed class ClientCookieDecoder : CookieDecoder
    {
        ...
        ClientCookieDecoder(bool strict)
            : base(strict)
        {
        }
        ...
        sealed class CookieBuilder
        {
            ...
            {
                if (CharUtil.RegionMatches(this.header, true, nameStart, CookieHeaderNames.Path, 0, 4))
                {
            ...
            {
                if (CharUtil.RegionMatches(this.header, true, nameStart, CookieHeaderNames.Domain, 0, 5))
                {
                    this.domain = this.ComputeValue(valueStart, valueEnd);
                }
                else if (CharUtil.RegionMatches(this.header, true, nameStart, CookieHeaderNames.Secure, 0, 5))
                {
                    this.secure = true;
                }
            }
            ...
            {
                if (CharUtil.RegionMatches(this.header, true, nameStart, CookieHeaderNames.Expires, 0, 7))
                {
                    this.expiresStart = valueStart;
                    this.expiresEnd = valueEnd;
                }
                else if (CharUtil.RegionMatches(this.header, true, nameStart, CookieHeaderNames.MaxAge, 0, 7))
                {
            ...
            {
                if (CharUtil.RegionMatches(this.header, true, nameStart, CookieHeaderNames.HttpOnly, 0, 8))
                {
                    this.httpOnly = true;
                }
            }

            static bool IsValueDefined(int valueStart, int valueEnd) => 
                valueStart != -1 && valueStart != valueEnd;

            string ComputeValue(int valueStart, int valueEnd) => 
                IsValueDefined(valueStart, valueEnd) 
